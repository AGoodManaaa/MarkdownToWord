...
class SmartIndent:
    ...
    def _handle_list_return(self, match, pattern_name: str, line: str, col: int):
        ...
        if not content:
            ...
            self._text.insert(f"{line_num}.0", indent[:-len(self._get_indent_unit())] if indent else '')
            return 'break'
...
class BracketMatcher:
    ...
    def _bind_events(self):
        """绑定键盘事件"""
        # 括号输入
        for char in BRACKET_PAIRS.keys():
            self._text.bind(f'<KeyPress-{char}>', 
                           lambda e, c=char: self._on_bracket_input(c), add='+')
        
        # 右括号输入
        for char in set(BRACKET_PAIRS.values()):
            if char not in BRACKET_PAIRS:  # 避免重复绑定
                self._text.bind(f'<KeyPress-{char}>',
                               lambda e, c=char: self._on_close_bracket(c), add='+')
    ...
    def _on_bracket_input(self, char: str):
        ...
        try:
            ...
            # 选中包裹后的内容
            new_end = f"{sel_start}+{len(selected) + 2}c"
        ...
        for offset in ['-1c', '']:
            try:
                ...
                if char in BRACKET_PAIRS:
                    ...
                    # 找配对的左括号
                    open_char = [k for k, v in BRACKET_PAIRS.items() if v == char][0]
